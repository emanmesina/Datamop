...
def show_terms_and_conditions():
    if not os.environ.get('APP_TERMS_ACCEPTED', False):
        ...
        agree_var = tk.IntVar()
        agree_checkbox = tk.Checkbutton(terms_window, height=2, text="I agree to all", variable=agree_var)
        agree_checkbox.pack()

        start_button = tk.Button(terms_window, height=2, width=15, text="Start Organizing", command=lambda: on_start_clicked(agree_var, terms_window))
        start_button.pack()
...
#-----------------
root.background_label = tk.Label(root)
root.background_label.place(x=0, y=0, relwidth=1, relheight=1)

# Set the background image
root.background_image = tk.PhotoImage(file="images/background.png")
root.background_label.configure(image=root.background_image)
#-----------------
...
directory_label = tk.Label(directory_label, bg="white", text=" No Folder Selected ", font=("Helvetica", 9, "italic"))
directory_label.pack()

browse_button = tk.Button(root, image=select_folder_img, command=on_browse_clicked, state="disabled", relief="flat", fg="light blue")
browse_button.pack()

organize_button = tk.Button(root, image=organize_img, command=on_organize_clicked, state="disabled", relief="flat", fg="light blue")
organize_button.pack()

root.after(500, show_terms_and_conditions)
root.mainloop()

directory_label = tk.Label(root, text="", font=("Helvetica", 11, "bold"), bg="light blue")
directory_label.pack()

browse_button = tk.Button(root, width=225, height=78, command=on_browse_clicked, font=("Helvetica", 11, "bold"), fg="light blue")
browse_button.configure(image=select_folder_img)
browse_button.pack()

organize_button = tk.Button(root, width=180, height=62, command=on_organize_clicked, font=("Helvetica", 11, "bold"), fg="light blue")
organize_button.configure(image=organize_img)
organize_button.pack()
